...
public class ProgramStudentsTable extends AbstractTable {
    ...
    public synchronized void insert(String programId, Collection<Student> students) {
        ...
        try {
            ...
            for (Student student : students) {
                try {
                    insert(db, programId, student);
                } catch (Exception e) {
                    Log.e(TAG, "Error while add program students", e);
                }

            }
            db.setTransactionSuccessful();
        } catch (Exception e) {
            Log.e(TAG, "Error while insert program students in Batch", e);
        } finally {
            db.endTransaction();
        }
    }

    private void insert(SQLiteDatabase db, String programId, Student student) {
    ...
    public Student getStudentByProgramIdAndStudentId(String programId) {
        final String query = "Select * from " + NAME + " where " + COLUMN_PROGRAM_ID + " = " + programId;
        ...
        try {
            ...
            Log.e(TAG, "Error while get program student", e);
        } finally {
            if (!cursor.isClosed()) {
                cursor.close();
            }
    ...
    private ArrayList<Student> getStudentByProgramId() {
